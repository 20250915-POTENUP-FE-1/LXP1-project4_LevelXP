$(function(){
    ...
    $(function(){
      $("#lecture-wrap").load("/bbs/lecture.html", function(){
        // 기존 동일 이벤트 제거(중복 바인딩 방지)
        $('#lecture-wrap').off('click.buyModal');

        // 오직 .lecture-list 내부의 각 리스트 항목 링크만 처리
        $('#lecture-wrap').on('click.buyModal', '.lecture-list ul li > a', function(e){
          e.preventDefault();
          ...
          if (!title) title = $link.find('.tit, .lecture-title').first().text().trim();
          if (!title) title = $link.attr('title') || $link.text().trim().slice(0,80) || '선택한 강의';
        ...
        // 닫기
        $(document).on('click.buyModalHandlers', '#confirmModal .btn-cancel, #confirmModal .overlay', function(){
          $('#confirmModal').fadeOut(120).attr('aria-hidden','true');
        });

        // 확인
        $(document).on('click.buyModalHandlers', '#confirmModal .btn-confirm', function(){
          $('#confirmModal').fadeOut(120).attr('aria-hidden','true');
          alert('구매가 완료되었습니다.');
        });
