  ...
  connectedCallback() {
    ...
    this.shadowRoot.innerHTML = `
      ...
      <div class="board-search">
        ...
        ${ isAdmin ? `<a href="#" class="btn-pack medium third btn-regist">강의등록</a>` : "" }
        <div class="search">
      ...
      <div id="lecture-list"><slot></slot></div>
    ...
    this.shadowRoot.addEventListener('click', (event) => {
      // 강의등록 버튼: 단순 open 요청
      const registBtn = event.target.closest && event.target.closest('.btn-regist');
      if (registBtn) {
        event.preventDefault();
        this.dispatchEvent(new CustomEvent('openCommonModal', {
      ...
      const editBtn = event.target.closest && event.target.closest('.btn-pack.small.edit, .btn-edit, a.btn-pack.small.edit');
      if (editBtn) {
        event.preventDefault();
        ...
        for (let i = 0; i < path.length; i++) {
          const node = path[i];
          if (node && node.nodeType === 1 && node.classList && node.classList.contains('course-card')) {
            card = node;
            break;
          }
        }
        ...
        // 기본 데이터
        let data = { index: '', category:'', name:'', instructor:'', price:'', cop:'', imgSrc:'' };
        if (card) {
          // data-index 우선
